...
@TRAINER_REGISTRY.register("rekt")
class ReKTTrainer(BaseTrainer):
    ...
    def _forward_batch(self, batch, train=False):
        ...
        preds = self.model(dcur, train=train)
        preds_for_loss = _align_preds(preds, rshft)
...
def _align_preds(preds, target):
    preds_for_loss = preds[:, 1:] if preds.size(1) == target.size(1) + 1 else preds
    if preds_for_loss.shape != target.shape:
        raise ValueError(
            f"ReKT prediction shape {tuple(preds.shape)} does not align with shifted targets {tuple(target.shape)}."
        )
