"""ExtraKT port for KT-Toolkit.

    extraKT: Extending Context Window of Attention Based Knowledge Tracing
    Models via Length Extrapolation.

Source: pykt-team/pykt-toolkit, pykt/models/extrakt.py (fetched 2026-09-18).
    ...
    if emb_type in ["qid"] != -1:
        ...
        with torch.no_grad():
            ...
            position_effect = torch.abs(
                x1-x2)[None, None, :, :].type(torch.FloatTensor).to(scores.device)  # 1, 1, seqlen, seqlen 
...
@MODEL_REGISTRY.register("extrakt")
class ExtraKT(extraKT):
