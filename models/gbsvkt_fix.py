...
@MODEL_REGISTRY.register("gbsvkt_fix")
class GBSVKTFix(nn.Module):
    """Granular Ball Support Vector Knowledge Tracing (Fixed).

    Fixes from GBSVKT:
    1. theta returns raw SVM margin (no sigmoid) for proper IRT a*(theta-b) computation
    2. pred1 returns logit before final sigmoid for consistent SVM-style training
    3. All prediction functions use consistent normalized margin computation
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # SVM-style prediction parameters with Xavier initialization
        self.W_b = nn.Linear(self.d_p, 1)
        self.W_a = nn.Linear(self.d_p, 1)

        # W_shortcut, b_shortcut: for final probability prediction
        self.W_shortcut = nn.Linear(4 * self.d_p, 1)
        self.b_shortcut = nn.Parameter(torch.zeros(1))

        # W_theta, b_theta: for theta (SVM margin), NO sigmoid here
        self.W_theta = nn.Linear(self.d_p, 1)
        self.b_theta = nn.Parameter(torch.zeros(1))

        # W_conf, b_conf: for confidence estimation
        self.W_conf = nn.Linear(self.d_p, 1)
        self.b_conf = nn.Parameter(torch.zeros(1))
    ...
    def ball_to_ball_predict(self, mu_h, r_h, mu_d, r_d):
        ...
        # theta: raw SVM margin (not sigmoided) for proper IRT computation
        theta_raw = self._normalized_margin(effective_diff, self.W_theta.weight, self.b_theta)
        theta = torch.sigmoid(theta_raw)  # keep sigmoid version for output compatibility

        # difficulty parameter b
        b = self.W_b(center_diff).squeeze(-1)

        # discrimination parameter a (positive)
        a = F.softplus(self.W_a(-radius_sum)).squeeze(-1)

        # confidence from radius sum
        conf = torch.sigmoid(self._normalized_margin(-radius_sum.unsqueeze(-1), self.W_conf.weight, self.b_conf))

        # IRT-style logit: a * (theta - b)
        # theta_raw is the raw margin (can be any real value), b is also real
        # Both theta and b should be in the same logit space for meaningful subtraction
        logit_irt = a * (theta_raw - b)

        # shortcut prediction (logit level)
        shortcut_input = torch.cat([mu_h_p, mu_d_p, center_diff, mu_h_p * mu_d_p], dim=-1)
        shortcut_logit = self._normalized_margin(shortcut_input, self.W_shortcut.weight, self.b_shortcut)

        # Final probability with IRT + shortcut combination
        logit = logit_irt + 0.1 * shortcut_logit
        p_hat = torch.sigmoid(logit)

        return {
            "p_hat": p_hat,
            "theta": theta,          # sigmoided for output (backward compat)
            "theta_raw": theta_raw,  # raw margin for trainer loss computation
            "b": b,
            "a": a,
            "confidence": conf,
            "mu_h_p": mu_h_p,
    ...
    def forward(self, q, c, r):
        if q is None or c is None:
            raise ValueError("GBSVKTFix requires both question and concept sequences.")
        ...
        if q.dim() != 2:
            raise ValueError(f"GBSVKTFix expects q shape [B, T], but got {tuple(q.shape)}")
        ...
        for t in range(seq_len - 1):
            ...
            theta_list.append(pred["theta"])  # sigmoided for output
            conf_list.append(pred["confidence"])
            r_h_list.append(r_h.mean(dim=-1))
            r_d_list.append(r_d_next.mean(dim=-1))
            feature1_list.append(pred["shortcut_input"])
            feature2_list.append(pred["theta_raw"])  # raw margin for trainer
            feature4_list.append(pred["effective_diff"])
        ...
        return {
            ...
            "feature1": torch.stack(feature1_list, dim=2),
            "feature2": torch.stack(feature2_list, dim=2),
            "feature4": torch.stack(feature4_list, dim=2)
        }
