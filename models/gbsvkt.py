...
class GBSVKT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # torch.nn.init.xavier_normal(self.W_shortcut)
        # torch.nn.init.xavier_normal(self.W_theta)
        # torch.nn.init.xavier_normal(self.W_conf)
        #
        # torch.nn.init.zeros_(self.b_shortcut)
        # torch.nn.init.zeros_(self.b_theta)
        # torch.nn.init.zeros_(self.b_conf)
    ...
    def ball_to_ball_predict(self, mu_h, r_h, mu_d, r_d):
        ...
        confidence = torch.sigmoid(self.pred4(self.W_conf, self.b_conf, -radius_sum).squeeze(-1))
        logit_irt = a * (theta - b)
        ...
        return {
            ...
            "feature1": shortcut_input,
            "feature2": effective_diff,
            "feature4": -radius_sum,
        }
    ...
    def pred1(self, w, b, logit_irt, y):
        ...
        norm_w = torch.norm(w)
        y = (torch.matmul(y, w) + b) / norm_w
        y = y.squeeze(-1)

        y = logit_irt + 0.1 * y

        # 预测概率计算（满足DKT需求）
        y = torch.sigmoid(y)  # 归一化距离，距离差越大预测越确信
    ...
    def forward(self, q, c, r):
        ...
        for t in range(seq_len - 1):
            ...
            feature1_list.append(pred["feature1"])
            feature2_list.append(pred["feature2"])
            feature4_list.append(pred["feature4"])
