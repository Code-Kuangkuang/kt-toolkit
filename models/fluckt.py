"""FlucKT port for KT-Toolkit.

    Cognitive Fluctuations Enhanced Attention Network for Knowledge Tracing,
    AAAI 2025.

Source: pykt-team/pykt-toolkit, pykt/models/fluckt.py (fetched 2026-09-18).
Official implementation: https://github.com/teemohold/FlucKT

The pykt model class is kept verbatim below; `FlucKTModel` at the end is the
thin adapter that registers it and renames the constructor arguments, exactly
as models/robustkt.py does.

Family note: `FrequencyLayer` splits each embedding into a low-pass component
(a CausalConv1d output, the "trend") and a high-pass residual (the
"fluctuation"), recombining them as `low + sqrt_beta**2 * high`. This is the
same filter as models/robustkt.py's trend/random split; models/mockt.py is the
same idea again with three length-bucketed kernels instead of one. Treat the
three as one family when reading their reported gains.

As in MoC-KT, the filter only runs when `emb_type` contains "conv", so the
registered default is "qid_conv"; with plain "qid" this file is AKT.

UNRESOLVED: pykt's own sweep for this model
(examples/seedwandb/fluckt.yaml) sets `emb_type: ["qid"]`, which by the gate in
`Framework.forward` turns the fluctuation filter OFF -- i.e. their published
sweep appears to train plain AKT. Their MoC-KT sweep, from the same authors,
uses "qid_conv_ker_noexp" and does enable it. This repo keeps the filter on and
refuses a conv-less emb_type, because silently training AKT under the name
FlucKT is the worse failure; but the discrepancy is not explained and any
comparison against the paper's numbers should account for it.
"""
...
# tests/test_model_contracts.py catches that on the CPU-only path. Every such
...
class FlucKT(nn.Module):
    ...
    def base_emb(self, q_data, target):
        ...
        # this, and FlucKT has to match or a comparison against them silently
...
@MODEL_REGISTRY.register("fluckt")
class FlucKTModel(FlucKT):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Accepted by sibling models' configs but not by FlucKT.__init__.
        ...
        if "conv" not in emb_type:
            raise ValueError(
                f"FlucKT's fluctuation filter only runs when emb_type contains "
                f"'conv' (Framework.forward gates on it); got {emb_type!r}, "
                f"which would silently train plain AKT."
            )
        if kernel_size < 2:
            # CausalConv1d slices `[..., :-padding]` with padding =
            # (kernel_size - 1); at kernel_size=1 that is `[:-0]`, which empties
            # the tensor instead of being a no-op.
            raise ValueError(
                f"FlucKT needs kernel_size >= 2; got {kernel_size}, which makes "
