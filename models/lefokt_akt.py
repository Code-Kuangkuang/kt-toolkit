    ...
    def __init__(
        ...
        **kwargs,
        ...
        if n_question is None:
            if num_c is None:
                raise ValueError("LEFOKT_AKT requires n_question or num_c.")
            n_question = num_c
        if n_pid is None:
            n_pid = 0 if num_q is None else num_q
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, d_feature, n_heads, dropout, kq_same, num_buckets,max_distance,init_c, init_L, bar_d, bias=True, emb_type="qid"):
        ...
        if emb_type.find("alibi") != -1:
            ...
            bias = self.slopes.unsqueeze(1).unsqueeze(1) * relative_position
            self.register_buffer("bias", bias.view(1, attn_heads, maxpos, maxpos), persistent=False)
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, gamma=None, pdiff=None,emb_type="qid",bias=None,bar_d=32):
    ...
    # RPEs choice
    if emb_type.find("alibi") != -1:
        scores = scores + bias[:, :, :seqlen, :seqlen].to(attn_device)
    elif emb_type.find("log") != -1:
        scores = bias(scores) 
    elif emb_type.find("power") != -1:
        scores = bias(scores) 
        ...
        with torch.no_grad():
            ...
            scores_ = scores_ * mask.float().to(attn_device) # 结果和上一步一样
            distcum_scores = torch.cumsum(scores_, dim=-1)  # bs, 8, sl, sl
            disttotal_scores = torch.sum(
                scores_, dim=-1, keepdim=True)  # bs, 8, sl, 1 全1
            # print(f"distotal_scores: {disttotal_scores}")
            position_effect = torch.abs(
                x1-x2)[None, None, :, :].float().to(attn_device)  # 1, 1, seqlen, seqlen 位置差值
