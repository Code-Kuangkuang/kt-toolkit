    ...
    def __init__(
        ...
        **kwargs,
        ...
        if skill_max is None:
            if num_c is None:
                raise ValueError("ReKT requires skill_max or num_c.")
            skill_max = int(num_c)
        if pro_max is None:
            if num_q is None:
                raise ValueError("ReKT requires pro_max or num_q.")
            pro_max = int(num_q)

        self.pro_max = int(pro_max)
        ...
        self.time_embed = nn.Parameter(torch.rand(self.seq_len, d))

        self.ls_state = nn.Parameter(torch.rand(1, d))#？？？？领域状态
        self.c_state = nn.Parameter(torch.rand(1, d))#？？？？未使用到

        state_len = max(self.seq_len - 1, 1)
        self.pro_state = nn.Parameter(torch.rand(state_len, d))
        self.skill_state = nn.Parameter(torch.rand(state_len, d))
        ...
        self.akt_pro_diff = nn.Parameter(torch.rand(self.pro_max, 1))
