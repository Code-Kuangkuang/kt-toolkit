"""FoLiBiKT port for KT-Toolkit.

    FoLiBiKT: Forgetting-aware Linear Bias for Attentive Knowledge Tracing,
    CIKM 2023.

Source: pykt-team/pykt-toolkit, pykt/models/folibikt.py (fetched 2026-09-18).

The pykt class is kept verbatim; the adapter at the end registers it, renames
the constructor arguments and applies this repo's multi-concept pooling, as
models/robustkt.py does.
"""
...
# tests/test_model_contracts.py catches that. Every such tensor now derives its
# device from a tensor already in scope, and the global is gone.
...
class folibiKT(nn.Module):
    ...
    def base_emb(self, q_data, target):
        ...
        # padding masked, as models/robustkt.py and models/stablekt.py do. Every
        # AKT-family model in this repo shares that protocol, or rows against
        # each other are not comparable.
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, d_feature, n_heads, dropout, kq_same, num_buckets,max_distance, bias=True, emb_type="qid"):
        ...
        maxpos = 1000
        attn_heads = n_heads  
        # Upstream hardcoded `.cuda()` on these three, so the model could not
        # run on CPU at all and would pin to device 0 on a multi-GPU box. They
        # are built on CPU and registered as buffers instead, which is also what
        # makes `model.to(device)` move them -- as plain attributes they stayed
        # put and tests/test_model_contracts.py caught the mismatch.
...
@MODEL_REGISTRY.register("folibikt")
class FoLiBiKT(folibiKT):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for key in ('device', 'dpath', 'num_at', 'num_it', 'seed'):
            kwargs.pop(key, None)
        if emb_size is None:
