...
class DTransformer(nn.Module):
    ...
    def embedding(self,  q_data, target, pid_data=None):
        ...
        if self.n_pid > 0: # have problem id
            q_embed_diff_data = pool_concept_embeddings(self.q_diff_embed, q_data, self.n_question)  # d_ct 总结了包含当前question（concept）的problems（questions）的变化
    ...
    def get_cl_loss(self, q, s, pid=None):
        ...
        for b in range(bs):
            ...
            # manipulate score
            idx = random.sample(
                range(cur_len), max(1, int(cur_len * self.dropout_rate))
            )
            for i in idx:
...
class DTransformerModel(DTransformer):
    def __init__(
        ...
        **kwargs,
        ...
        for key in ("seq_len", "device", "dpath", "num_at", "num_it"):
            kwargs.pop(key, None)
        if emb_size is None:
            emb_size = d_model if d_model is not None else 128
        if num_pid is None:
            num_pid = num_q
        super().__init__(
            n_question=num_c,
