...
def main(
    ...
    test_file: Path = typer.Option(ROOT / "data" / "aaai2023" / "pykt_test.csv", "--test-file"),
    output: Path = typer.Option(ROOT / "prediction.csv", "--output"),
    dataset_name: str = typer.Option("aaai2023", "--dataset-name"),
    ...
    if model_name not in {"iekt", "akt"}:
        raise typer.BadParameter("scripts/predict_aaai2023.py currently supports IEKT and AKT checkpoints.")
